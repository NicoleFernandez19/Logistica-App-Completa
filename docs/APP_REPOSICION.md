...
1. **Los 222 siguen en el maestro actual (Septiembre) — ninguno se dio de baja.** Si un agente hubiera cerrado, el propio cálculo ya lo neutraliza solo: al no estar en el maestro más reciente, su `SUBSEGMENTACION` llega vacía → se rellena en 0 (no en 1, ver "SUBSEGMENTACION como multiplicador") → reposición 0 en todos los productos para ese agente. Esto es automático, no depende de qué tipo de join se use para el historial.
