...
def calcular(tiv, maestro, fac_termicas, prisma_env, sube_env, trx_sube,
    ...
    consumo = df[["ID_PF", "ROLLOS", "ROLLO_PRISMA", "ROLLO_SUBE",
                  "BOLSAS_RECOLECCION"]].copy()
    m = m.merge(consumo, on="ID_PF", how="left")

    for c in ["ENVIO_PRISMA", "ENVIO_SUBE", "ENVIO_ROLLO",
              "ROLLOS", "ROLLO_PRISMA", "ROLLO_SUBE", "BOLSAS_RECOLECCION"]:
        m[c] = m[c].fillna(0)

    for col in ["STOCK_ROLLO_ANT", "STOCK_SUBE_ANT", "STOCK_PRISMA_ANT"]:
    ...
    m["AGENTE_NEGATIVO"] = (m["STOCK_ROLLO"] < 0).astype(int)
...
def preparar_maestro_exportable(df_tiv, df_maestro):
    ...
    # Columnas que no calcula App 1 → default 0
    ...
    num_cols = ["STOCK ROLLO", "STOCK SUBE", "STOCK PRISMA",
                "ROLLO", "BOLSA RECOLECCION",
